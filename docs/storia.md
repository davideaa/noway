# La storia del progetto — l'archivio

Qui sta tutto quello che **è stato deciso e non si rifà**: strategie
scartate, errori commessi, versioni superate, e il racconto di come si
è arrivati allo stato attuale.

**Non serve leggerlo per lavorare.** Lo stato corrente sta in
`docs/CONTINUA-QUI.md`, che è corto apposta. Questo file si apre quando
serve sapere *perché* una cosa è come è, o quando si è tentati di
ritentare qualcosa che è già stato bocciato.

Niente qui è stato cancellato o riassunto: è il testo originale,
spostato fuori dalla strada il 2026-09-21. Le sezioni tengono la
numerazione che avevano, così i rimandi vecchi continuano a funzionare.
...
# 6. Cosa fare adesso — la versione di settembre (SUPERATA)

> Sostituita dalla lista in `docs/CONTINUA-QUI.md`. Tenuta perché
> il punto 4 (confrontare swap e spread veri prima del live) non è
> stato ancora fatto e vale ancora.
...
# 10. I parametri e i numeri, versione di settembre

Testo originale delle sezioni 1 e 2, tenuto perché contiene misure che
non sono state rifatte: la tabella dei parametri con il motivo di ogni
scelta, il comportamento per regime di mercato, i costi.

> **I numeri sul rischio qui sotto sono SUPERATI.** Dicevano 0,70% e
> tetto 35% al 90° percentile. Il valore giusto è **oro 0,65% · nasdaq
> 0,98%**, con tetto 33% al 95°, che è già sforato. Vedi
...
# 9. Cosa è stato costruito a settembre 2026

- **Pannello live** su tutti e due gli EA: diagnostica, PnL giorno /
  settimana / mese in denaro e in % sul saldo a inizio periodo,
  drawdown massimo, rendimento composto al netto dei versamenti,
  rischio effettivo, posizione aperta. **Si spegne da solo nel tester.**
...
- **Sei report nuovi** in `report/` e **sei strumenti nuovi** in
  `tools/`, elencati in `docs/indice.md`.

## Il risultato dell'analisi dei regimi

**Zero variabili significative su 25 provate.** L'edge non dipende dal
regime, e la differenza fra gli anni **non è distinguibile dal caso**
(oro p 0,37, nasdaq p 0,24): il timore che il boom 2024-2026 spieghi
tutto **non è confermato**.

Unica eccezione, e regge il fuori campione: l'oro soffre nei mercati
**laterali a volatilità media** (−0,11 R, PF 0,82, n 112) e concentra i
grandi vincitori nei periodi direzionali e calmi (p 0,013).
Correlazione della classifica dei regimi dentro/fuori campione: oro
**+0,613**, nasdaq **−0,762** (= nessuna relazione).

Il dettaglio sta in `docs/analisi-regimi.md`.
