//+------------------------------------------------------------------+
//|                                          GoldTrendPullback.mq5   |
//|                                                                  |
//|  CANDIDATA 6 — Ingresso sul ritracciamento dentro il trend (H4)   |
//|                                                                  |
//|  PERCHE' ESISTE                                                   |
//|  Il range mean reversion e' stato bocciato: 174 configurazioni    |
//|  con almeno 100 trade, zero in utile. L'errore era cercare una    |
//|  gamba "laterale" perche' e' la terza casella di uno schema       |
//|  teorico. Qui si diversifica per DURATA, non per tipo di mercato. |
//|                                                                  |
//|  IPOTESI, scritta prima di qualunque test:                        |
//|  l'oro e' un mercato che tende, ma sporco: parte, ritraccia,      |
//|  riparte. Un sistema veloce viene stoppato dal ritracciamento e   |
//|  perde il seguito del movimento. Un sistema lento resta dentro.   |
//|  Entrare DOPO il ritracciamento da' due cose: uno stop molto piu' |
//|  vicino (quindi trade piu' grandi in R quando va bene) e un       |
//|  momento di ingresso diverso da quello di S3.                     |
//|                                                                  |
//|  DECORRELAZIONE DA S3, meccanica:                                 |
//|  S3 entra quando il prezzo fa un nuovo estremo a 60 barre M30.    |
//|  Qui si entra solo se il prezzo NON e' sull'estremo: serve un     |
//|  ritracciamento di almeno N ATR dal massimo recente. Le due non   |
//|  possono comprare sulla stessa candela.                           |
//|  E l'orizzonte e' diverso: S3 tiene ore, questa tiene settimane.  |
//|                                                                  |
//|  REGOLE (long; short speculare)                                   |
//|  1. trend su: chiusura sopra l'EMA e EMA in salita                |
//|  2. massimo recente su InpSwingBars barre                         |
//|  3. il prezzo e' ritracciato di almeno InpPullbackATR ATR         |
//|     da quel massimo, e NON e' sul massimo                         |
//|  4. ripartenza: la barra chiude sopra il massimo della precedente |
//|  5. stop sotto il minimo del ritracciamento, meno un buffer       |
//|  6. nessun take profit: trailing largo, si cavalca                |
//|     (il target fisso amputa la coda destra: gia' misurato su S3)  |
//|                                                                  |
//|  Long e short hanno interruttori separati: sull'oro, che sale nel |
//|  lungo periodo, e' plausibile che non rendano uguale. Da misurare,|
//|  non da assumere.                                                 |
//|                                                                  |
...
//|                                                                  |
//|  CRITERI DI PROMOZIONE, dichiarati prima del test:                |
//|  almeno 150 trade, profit factor >= 1,20, t >= 3,4.               |
//|  Se non li centra si butta.                                       |
//+------------------------------------------------------------------+
...
input long              InpMagic            = 998000;    // Magic number
...
input int               InpTrendEma         = 60;        // <<< Periodo EMA di trend
...
input double            InpStopBufferATR    = 0.50;      // <<< Stop oltre il minimo, in ATR
input double            InpTrailStartR      = 1.0;       // Attiva trailing a +xR
...
bool OpenPullback(const bool isLong, const double stopPrice)
  {
   ...
   // nessun take profit: si cavalca con il trailing
   bool ok = isLong ? trade.Buy (lots, _Symbol, 0.0, sl, 0.0, "PULLBACK-LONG")
                    : trade.Sell(lots, _Symbol, 0.0, sl, 0.0, "PULLBACK-SHORT");
   if(ok)
     {
...
void PrintSummary()
  {
   ...
   if(n == 0) { Print("=== PULLBACK: nessun trade ==="); return; }

   double pf = (gl != 0.0) ? gw / MathAbs(gl) : 0.0;
   double al = (n - w > 0) ? gl / (n - w) : 0.0;
   PrintFormat("=== PULLBACK NEL TREND (H4) ===");
